...
at::Tensor ROIAlign_forward_cuda(const at::Tensor& input,
                                 ...
                                 const int sampling_ratio) {
  AT_ASSERTM(input.scalar_type().is_cuda(), "input must be a CUDA tensor");
  AT_ASSERTM(rois.scalar_type().is_cuda(), "rois must be a CUDA tensor");
...
at::Tensor ROIAlign_backward_cuda(const at::Tensor& grad,
                                  ...
                                  const int width,
                                  const int sampling_ratio) {
  AT_ASSERTM(grad.scalar_type().is_cuda(), "grad must be a CUDA tensor");
  AT_ASSERTM(rois.scalar_type().is_cuda(), "rois must be a CUDA tensor");
